...
class JSON
{
    ...
    public:
        ...
        static JSON LoadFromCached(std::vector<uint8_t> &cached, int &p);
        ...
        void erase( const string &key ) const {
        ...
        void save(std::vector<uint8_t> &out) const {
            switch( Type ) {
                case Class::Null:
                    out.push_back(SaveNull);
                    break;
                case Class::Integral:
                    out.push_back(SaveInt);
                    for(int i = 0; i < 8; i++) out.push_back((Internal.Int >> ((7 - i) * 8)) & 0xFF);
                    break;
                case Class::Floating:
                    out.push_back(SaveFloat);
                    for(int i = 0; i < 8; i++) out.push_back((*((long*)&Internal.Float) >> ((7 - i) * 8)) & 0xFF);
                    break;
                case Class::Boolean:
                    out.push_back(Internal.Bool ? SaveTrue : SaveFalse);
                    break;
                case Class::Array:
                    out.push_back(SaveArray);
                    for(int i = 0; i < length(); i++) {
                        Internal.List->at(i).save(out);
                    }
                    out.push_back(SaveArrayEnd);
                    break;
                case Class::Object:
                    out.push_back(SaveObject);
                    for(auto &entry : ObjectRange()) {
                        save_string(out, entry.first);
                        entry.second.save(out);
                    }
                    out.push_back(SaveObjectEnd);
                    break;
                case Class::String:
                    out.push_back(SaveString);
                    save_string(out, *Internal.String);
                    break;
            }
        }
...
inline std::string cached_read_string(std::vector<uint8_t> &cached, int &p) {
    std::string value((char*)&cached[p++]);
    while(cached[p++] != 0); // Advance past the null terminator
    return value;
}

inline JSON JSON::LoadFromCached(std::vector<uint8_t> &cached, int &p) {
    JSON json;

    int saveType = cached[p++];
    ...
    switch(saveType) {
        ...
        case SaveInt:
            {
                long val = 0;
                for(int i = 0; i < 8; i++) val |= cached[p++] << (7 - i) * 8;
                json = val;
            }
            break;
        case SaveFloat:
            {
                double val = 0;
                for(int i = 0; i < 8; i++) *((long*)&val) |= cached[p++] << (7 - i) * 8;
                json = val;
            }
            break;
        case SaveTrue:
        ...
        case SaveArray:
            ...
            while(cached[p] != SaveArrayEnd) {
                json.append(LoadFromCached(cached, p));
            }
            p++;
            break;
        case SaveObject:
            json = JSON::Make( JSON::Class::Object );
            while(cached[p] != SaveObjectEnd) {
                std::string key = cached_read_string(cached, p);
                json[key] = LoadFromCached(cached, p);
            }
            p++;
            break;
        case SaveString:
            json = cached_read_string(cached, p);
            break;
    }
